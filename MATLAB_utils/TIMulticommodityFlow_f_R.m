function [cplex_out,numvehicles] = TIMulticommodityFlow_f_R(RoadGraph,RoadCap,TravelTimes,Sources,Sinks,FlowsIn,FlowsOut,milpflag,congrelaxflag,sourcerelaxflag,debugflag)

% Forked from TIMulticommodityFlow_f on Feb. 2, 2017
% Specifically for rebalancing flows. Does not assume that number of
% sources equals number of sinks. Will give unexpected results for actual
% multicommodity flows, DO NOT USE for those.
...
N=length(RoadGraph);
M=max(size(Sources,1),size(Sinks,1));

S_so=size(Sources,2); 
S_si=size(Sinks,2);

if M>1
   disp('WARNING: this will probably not work for multicommodity flows. See the readme. Press a key to continue or ctrl-c to exit.')
   pause
end
...
f_cost(SoRelaxkl(1,1) : SoRelaxkl(M,S_so))=SourceCost;
f_cost(SiRelaxkl(1,1) : SiRelaxkl(M,S_si))=SourceCost;
...
n_eq_entries = 2*E*(M)+ S_so*M + S_si*M;
...
for k=1:M
    ub(SoRelaxkl(k,1):SoRelaxkl(k,S_so))=FlowsIn(k,:)';
    ub(SiRelaxkl(k,1):SiRelaxkl(k,S_si))=FlowsOut(k,:)';
end
if (debugflag)
